...
/* wifi details. values are stored in secret.h */
#define WIFI_SSID         SECRET_WIFI_SSID 
#define WIFI_PASS         SECRET_WIFI_PASS
/* keys and IDs for sinric pro. values are stored in secret.h */
#define APP_KEY           SECRET_APP_KEY
#define APP_SECRET        SECRET_APP_SECRET
#define SWITCH_ID         SECRET_SWITCH_ID

#define BAUD_RATE         9600                // Change baudrate to your need

#define BUTTON_PIN 0   // GPIO for BUTTON. if grounded on boot, will put ESP into programming mode. (maybe drain the switch through an NPN transistor turned high by the ESP. that way, it can't be grounded until after booting)
...
void handleButtonPress() {
  ...
  if (digitalRead(BUTTON_PIN) == HIGH && actualMillis - lastBtnPress > 1000)  { // is button pressed and debounced?
    if (myPowerState) {     // flip myPowerState: if it was true, set it to false, vice versa
      myPowerState = false;
    } else {
      myPowerState = true;
    }
    //digitalWrite(BUILTIN_LED, myPowerState?LOW:HIGH); // if myPowerState indicates device turned on: turn on led (builtin led uses inverted logic: LOW = LED ON / HIGH = LED OFF)
    digitalWrite(LED_PIN, myPowerState);
